...
int scale(int hout, int wout, const kernel::base *pre, 
	...
	const extension::base *ext, image::rgba<float> *rgba, unsigned char *output)
{
    int hin, win;
    rgba->size(&hin, &win);
    ...
    fprintf(stderr, "  applying discrete postfilter\n");
    fprintf(stderr, "    i.e., %s*%s^-1\n", fir->name(), ifir->name());
    ...
    if (output) {
      //  fprintf(stderr, "saving %s\n", output);
        if (!image::store(output, (*rgba)))
            errorf(("error saving image '%s'", output));
    }
    return 1;
}
