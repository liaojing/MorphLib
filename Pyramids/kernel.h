...
namespace kernel {
    ...
    static inline void build_name(const discrete::base *fir, 
        ...
        if (discrete::isdelta(fir)) {
			if (discrete::isdelta(ifir)) strcpy_s(name, strlen(ker->name()),ker->name());
            else sprintf_s(name, 255, "%.30s*%.30s^(-1)", ker->name(), ifir->name());
        } else if (discrete::isdelta(ifir)) {
            sprintf_s(name, 255,"%.30s*%.30s", ker->name(), fir->name());
        } else {
            sprintf_s(name, 255,"%.30s*%.30s*%.30s^(-1)", ker->name(),
                fir->name(), ifir->name());
        }
    }
    ...
    public:
        ...
        generalized(const generalized &other): cloner_base<generalized>() {
            ...
            strcpy_s(m_name, 255, other.m_name);
        }
    private:
        char m_name[256];
        generating::base *m_ker;
